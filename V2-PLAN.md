...
### HANDOVER: dashboard narrative refresh (assigned to Codex, 2026-07-08)

Goal: bring the dashboard's narrative copy in line with the shipped
positioning (index.html and README are already updated; the dashboard is the
middle step of the reader path index -> dashboard -> README and still carries
the old framing).

Facts an executor must know before touching anything:
1. The dashboard HTML under docs/ is GENERATED by
   `py src/kpi/export_html_dashboard.py`. Never hand-edit the generated HTML;
   change the copy inside the export script and re-export. index.html is the
   owner's hand-tuned narrative and is NOT part of this task.
2. Owner approval gate: draft the new narrative copy first (headings, ledes,
   per-chart takeaway lines) and show it to the owner for approval BEFORE
   implementing. The owner's standing rule: the dashboard is evidence, not a
   chart pile; every chart carries a one-line takeaway.
3. Alignment target: the index.html positioning ("a fab simulation and
   decision engine built for AI agents to call"; three core highlights;
   modules M7 to M11 shipped). Mention new evidence (yield layer, equipment
   health, dispatching, agent memos, reliability gates) only where a chart or
   number on the dashboard actually backs it; adding M7-M11 charts is allowed
   only with owner-approved takeaways (static PNGs exist under
   reports/figures/ prefixed m7_ through m11_).
4. Style: English (plus Chinese only if the dashboard already carries the
   bilingual toggle; inspect first); zero em dash characters; no reader-
   visible V1/V2 version labels.
5. Verify: re-export, confirm zero em dash in the regenerated HTML, confirm
   index.html links to the dashboard still resolve, and run the fourteen
   check scripts listed in AGENTS.md (they must stay green; this task should
   not touch src/ outside src/kpi).
6. Version control: work on branch `v2`, Conventional Commits, push, then a
   PR to main for the owner to merge.
