...
Validation commands (run after touching generator/monitoring/quality code; use
`py`, not `python` — bare `python` is the broken Windows Store stub on this
machine): `py src/generator/validate_m2.py` · `py src/generator/crn_check.py` ·
`py src/monitoring/monitoring_check.py` · `py src/quality/quality_check.py` ·
`py src/quality/vm_check.py`
