...
Shared operating rules live in `F:\ai-dev-standards\`, shared with Claude Code
(via this project's CLAUDE.md). Read the file whose trigger applies; do not
paraphrase from memory. Change a shared rule in ai-dev-standards once, not per
project.
...
> **Codex note:** `10-model-dispatch.md` and `30-delegation-templates.md` are
> written for Claude Code's Agent tool (model = haiku/sonnet/opus; Explore/Plan
> agent types). Read them for the portable principles: delegate grunt work,
> attach a three-part work order (goal and why / acceptance criteria / report
> format), verify with a fresh context, never self-verify. Map those onto
> Codex's own sub-agent and model controls.
...
The public narrative is `docs/index.html`; `src/kpi/export_html_dashboard.py`
now exports the baseline Plotly HTML page only and removes the retired
`docs/dashboard.html` page if present. No Streamlit.

## Status

Status is tracked in the README roadmap table and `git log`, not in this file.
Current assigned handover for Codex / Claude Code: see `V2-PLAN.md` and read
the newest "HANDOVER" section in full before starting, unless the owner gives
a newer request directly.
